...
for movie in constants.SEARCH_LIST:
    search_id = search.Search(constants.SEARCH_URL.format(api_key=constants.API_KEY, query=movie))  # NOQA
    movie_id = search_id.get_id()

    search_info = search.Search(constants.INFO_URL.format(api_key=constants.API_KEY, movie_id=movie_id))  # NOQA
    movie_json = search_info.search_movie_info()

    get_movie_list(json.loads(movie_json), movies)
